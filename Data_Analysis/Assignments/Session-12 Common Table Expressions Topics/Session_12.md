...
'''
  **Syntax**
    ...
    ' WITH TopArtists AS ( SELECT artist_id, name, followers FROM SpotifyArtists ORDER BY followers DESC LIMIT 3 ) SELECT * FROM TopArtists; '
...
'''
  **Syntax**
    ...
    ' WITH MonthlyTotals AS ( SELECT DATE_FORMAT(order_date, '%m') AS order_month, SUM(total_amount) AS total_sales FROM FlipkartOrders WHERE YEAR(order_date) = 2023 GROUP BY DATE_FORMAT(order_date, '%m') ) SELECT order_month, total_sales FROM MonthlyTotals ORDER BY total_sales DESC LIMIT 1; '
...
'''
  **Syntax**
    ...
    ' WITH RECURSIVE CalendarDays AS ( SELECT CURRENT_DATE() AS day_date UNION ALL SELECT DATE_ADD(day_date, INTERVAL 1 DAY) FROM CalendarDays WHERE day_date < DATE_ADD(CURRENT_DATE(), INTERVAL 6 DAY) ) SELECT day_date FROM CalendarDays; '
...
'''
  **Syntax**
    ...
    ' WITH CityAvgRating AS ( SELECT city, AVG(rating) AS avg_rating FROM ZomatoRestaurants GROUP BY city ) SELECT r.* FROM ZomatoRestaurants r JOIN CityAvgRating c ON r.city = c.city WHERE c.avg_rating > 4.0; '
